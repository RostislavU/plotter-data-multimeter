# -*- coding: utf-8 -*-
"""
Скрипт построения графиков из .csv по заданному формату
"""
...
with open(settings['format']) as f:
    ...
    focus_numbers = list(
        map(lambda item: item.get('value'), focus_keys.values()))
    focus_numbers += list(
        filter(lambda item: item,
               map(lambda item: item.get('task'), focus_keys.values())))
    ...
    CHANEL1 = {
        ...
        'value_label': focus_keys.get('chanel1',
                                      None).get('value_label', None)
    }
    ...
    CHANEL2 = {
        ...
        'value_label': focus_keys.get('chanel2',
                                      None).get('value_label', None)
    }
    ...
    for fname in filenames:
        ...
        focus_columns = list(
            map(
                lambda x: x[1],
                list(
                    filter(lambda item: int(item[0]) in focus_numbers,
                           columns.items()))))
        ...
        for column in {
                CHANEL1['task'], CHANEL1['value'], CHANEL2['task'],
                CHANEL2['value']
        }:
            if column:
                # Перевести во float
        ...
        if settings['drop_value']:
            ...
            data = data.drop(
                data[data[CHANEL1['value']] > borders['max_chanel1']].index)
            data = data.drop(
                data[data[CHANEL1['value']] < borders['min_chanel1']].index)

            if CHANEL1['task']:
                data = data.drop(
                    data[data[CHANEL1['task']] > borders['max_chanel1']].index)
                data = data.drop(
                    data[data[CHANEL1['task']] < borders['min_chanel1']].index)

            if CHANEL2['task']:
                data = data.drop(
                    data[data[CHANEL2['task']] > borders['max_chanel2']].index)
                data = data.drop(
                    data[data[CHANEL2['task']] < borders['min_chanel2']].index)

            if CHANEL2['value']:
                data = data.drop(data[
                    data[CHANEL2['value']] > borders['max_chanel2']].index)
                data = data.drop(data[
                    data[CHANEL2['value']] < borders['min_chanel2']].index)
        ...
        if not frm['one_scale']:
            ...
            data[CHANEL1['value']].plot(ax=axes[0],
                                        color="orange",
                                        label=CHANEL1['value_label'])
            if CHANEL1['task']:
                data[CHANEL1['task']].plot(ax=axes[0],
                                           color='#FF7600',
                                           linestyle='--',
                                           label=CHANEL1['task_label'])
            ...
            data[CHANEL2['value']].plot(ax=axes[1],
                                        color="blue",
                                        label=CHANEL2['value_label'])
            if CHANEL2['task']:
                data[CHANEL2['task']].plot(ax=axes[1],
                                           color='#009999',
                                           linestyle='--',
                                           label=CHANEL2['task_label'])
            ...
            for CHANEL in (CHANEL1, CHANEL2):
                data[CHANEL['value']].plot(ax=axes,
                                           color=CHANEL['color'],
                                           label=CHANEL['value_label'])
